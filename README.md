...
- условный оператор `if` для сравнения,
- функция `swap()` для обмена.

**Временная сложность**: **O(n²)**  
**Почему O(n²)**: Два вложенных цикла, где внешний выполняется `n` раз, а внутренний — почти `n` раз на каждой итерации (в худшем случае). Для каждого элемента алгоритм просматривает всю оставшуюся часть массива, чтобы найти минимум. Это приводит к тому, что общее число операций пропорционально `n × n`, то есть квадратично зависит от размера входных данных.
...
**Временная сложность**: **O(n²)**  
**Почему O(n²)**: Два вложенных цикла: внешний определяет, сколько раз пройти по массиву (`n−1` раз), а внутренний на каждом проходе сравнивает почти все соседние пары. Даже если массив уже отсортирован, базовая версия не проверяет, были ли обмены, и всё равно выполняет все проходы. Общее число сравнений растёт как квадрат от длины массива.
...
**Временная сложность**: **O(n²)**  
**Почему O(n²)**: В худшем случае (массив в обратном порядке) каждый новый элемент приходится сравнивать со всеми предыдущими и сдвигать их. Это означает, что для `n` элементов может потребоваться до `n` операций на каждый, что в сумме даёт квадратичное число действий.
...
**Почему O(n log n)**: Массив делится пополам до тех пор, пока не останутся одиночные элементы — это требует примерно `log n` уровней рекурсии. На каждом уровне выполняется полное слияние всех элементов, что занимает время, пропорциональное `n`. Поскольку таких уровней `log n`, общее время — `n × log n`.
...
**Временная сложность**: **O(n²)**  
**Почему O(n²)**: При использовании последовательности шагов, начинающейся с `n/2` и уменьшающейся вдвое, алгоритм в худшем случае может выполнять почти столько же сравнений, сколько и обычная сортировка вставками. Это происходит, если данные плохо распределены относительно выбранных шагов, и на последнем проходе (`gap = 1`) приходится делать почти квадратичное число сдвигов.
...
**Почему O(n log n) в среднем**: Если опорный элемент делит массив примерно пополам, глубина рекурсии будет логарифмической, а на каждом уровне обрабатывается весь массив — отсюда линейно-логарифмическое время.  
**Почему O(n²) в худшем случае**: Если опора постоянно оказывается самым большим или самым маленьким элементом (например, в уже отсортированном массиве), деление становится крайне несбалансированным: один подмассив пустой, другой — почти весь исходный. Тогда глубина рекурсии становится линейной, и общее время — квадратичным.
...
**Временная сложность**: **O(n)**  
**Почему O(n)**: В худшем случае (когда искомый элемент отсутствует или находится в самом конце) алгоритм вынужден проверить каждый элемент массива по одному. Поэтому время выполнения прямо пропорционально количеству элементов.
...
**Почему O(log n)**: На каждом шаге алгоритм отбрасывает ровно половину оставшихся элементов. Это означает, что количество шагов, необходимых для сужения области поиска до одного элемента, растёт очень медленно — пропорционально логарифму от размера массива.
...
**Почему O(log log n) в лучшем случае**: Если элементы распределены почти равномерно, формула очень точно предсказывает позицию искомого элемента, и область поиска сужается гораздо быстрее, чем в бинарном поиске.  
**Почему O(n) в худшем случае**: Если распределение неравномерно (например, экспоненциальное), предсказания становятся неточными, и алгоритм может деградировать до линейного поиска, проверяя почти все элементы.
...
**Временная сложность**: **O(log n)**  
**Почему O(log n)**: Числа Фибоначчи растут экспоненциально, поэтому количество шагов, необходимых для покрытия всего массива, пропорционально логарифму от его длины. На каждом шаге отбрасывается часть массива, аналогично бинарному поиску, но с неравными частями. В итоге общее число сравнений остаётся логарифмическим.
